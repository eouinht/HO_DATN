import numpy as np


class NoHandoverAgent:
    def __init__(self, env):
        self.num_RUs = int(env.num_RUs)
        self.num_DUs = int(env.num_DUs)
        self.num_CUs = int(env.num_CUs)

        # Giới hạn lớn nhất toàn hệ thống, ví dụ 20 RB.
        # Giá trị thực tế cho từng UE sẽ lấy theo slice.
        self.max_RBs_per_UE = int(
            env.resource_manager.max_RBs_per_UE
        )

        self.power_levels = np.asarray(
            env.resource_manager.P_ib_sk_val,
            dtype=float,
        )

        if self.power_levels.size == 0:
            self.power_levels = np.asarray(
                [0.0],
                dtype=float,
            )

    def _get_slice_max_rbs(self, ue):
        """
        Lấy số RB tối đa theo slice của UE.

        Ví dụ:
            eMBB  -> 20 RB
            uRLLC -> 10 RB

        Nếu UE chưa có trường max_RBs thì dùng giới hạn
        chung của resource manager để tránh lỗi.
        """
        return int(
            ue.get(
                "max_RBs",
                self.max_RBs_per_UE,
            )
        )

    def select_action(self, state):
        active_ues = [
            (int(uid), ue)
            for uid, ue in state["UE_requests"].items()
            if int(
                ue.get(
                    "status",
                    {},
                ).get(
                    "active",
                    0,
                )
            ) == 1
        ]

        if not active_ues:
            return None

        ue_id, ue = active_ues[
            np.random.randint(
                len(active_ues)
            )
        ]

        alloc = ue.get(
            "allocation",
            {},
        )

        slice_max_rbs = self._get_slice_max_rbs(
            ue
        )

        # =================================================
        # CASE 1: UE đã được accept
        # Giữ nguyên hoàn toàn mapping và tài nguyên.
        # =================================================
        if alloc.get("RU") is not None:
            return (
                int(ue_id),
                0,  # Không handover
                int(alloc["RU"]),
                int(alloc["DU"]),
                int(alloc["CU"]),
                int(alloc["num_RB_alloc"]),
                float(alloc["power_alloc"]),
            )

        # =================================================
        # CASE 2: UE chưa được accept
        # Gán mapping và tài nguyên mới.
        # =================================================
        num_RB_alloc = int(
            np.random.randint(
                1,
                slice_max_rbs + 1,
            )
        )

        power_alloc = float(
            np.random.choice(
                self.power_levels
            )
        )

        return (
            int(ue_id),
            0,  # Không handover
            int(
                np.random.randint(
                    0,
                    self.num_RUs,
                )
            ),
            int(
                np.random.randint(
                    0,
                    self.num_DUs,
                )
            ),
            int(
                np.random.randint(
                    0,
                    self.num_CUs,
                )
            ),
            int(num_RB_alloc),
            float(power_alloc),
        )